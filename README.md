...
[![NPM version](https://img.shields.io/npm/v/workspace-financial-backend-sdk.svg)](https://npmjs.org/package/workspace-financial-backend-sdk) ![npm bundle size](https://img.shields.io/bundlephobia/minzip/workspace-financial-backend-sdk)
...
The REST API documentation can be found on [solomon-ai.app](https://solomon-ai.app). The full API of this library can be found in [api.md](api.md).
...
```sh
npm install git+ssh://git@github.com:stainless-sdks/workspace-financial-backend-sdk-typescript.git
```

> [!NOTE]
> Once this package is [published to npm](https://app.stainless.com/docs/guides/publish), this will become: `npm install workspace-financial-backend-sdk`
...
1. Via the `WORKSPACE_FINANCIAL_BACKEND_SDK_LOG` environment variable
...
const client = new WorkspaceFinancialBackendSDK({
  logger: logger.child({ name: 'WorkspaceFinancialBackendSDK' }),
...
const client = new WorkspaceFinancialBackendSDK({ fetch });
```
...
```ts
import WorkspaceFinancialBackendSDK from 'npm:workspace-financial-backend-sdk';
...
We are keen for your feedback; please open an [issue](https://www.github.com/stainless-sdks/workspace-financial-backend-sdk-typescript/issues) with questions, bugs, or suggestions.
