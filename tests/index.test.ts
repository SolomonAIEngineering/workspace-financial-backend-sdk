...
import { APIPromise } from '@solomon-ai/workspace-financial-backend-sdk/core/api-promise';
...
describe('logging', () => {
  ...
  beforeEach(() => {
    process.env = { ...env };
    process.env['WORKSPACE_FINANCIAL_BACKEND_SDK_LOG'] = undefined;
  });
  ...
  const forceAPIResponseForClient = async (client: WorkspaceFinancialBackendSDK) => {
  ...
  test('warn when env var level is invalid', async () => {
    ...
    const client = new WorkspaceFinancialBackendSDK({ logger: logger, bearerToken: 'My Bearer Token' });
    expect(client.logLevel).toBe('warn');
    expect(warnMock).toHaveBeenCalledWith('process.env[\'WORKSPACE_FINANCIAL_BACKEND_SDK_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]');
  });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new WorkspaceFinancialBackendSDK({ baseURL: 'http://localhost:5000/custom/path/', bearerToken: 'My Bearer Token' });
    ...
    afterEach(() => {
      process.env['WORKSPACE_FINANCIAL_BACKEND_SDK_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new WorkspaceFinancialBackendSDK({ baseURL: 'https://example.com', bearerToken: 'My Bearer Token' });
    ...
    test('in request options overridden by client options', () => {
      const client = new WorkspaceFinancialBackendSDK({ bearerToken: 'My Bearer Token', baseURL: 'http://localhost:5000/client' });
