...
const environments = {
  production: 'https://engine.solomon-ai-platform.com',
  environment_1: 'https://engine-staging.solomon-ai-platform.com',
  environment_2: 'http://localhost:8787',
};
type Environment = keyof typeof environments;

export interface ClientOptions {
  /**
   * Defaults to process.env['WORKSPACE_FINANCIAL_BACKEND_SDK_BEARER_TOKEN'].
   */
   ...
   * - `production` corresponds to `https://engine.solomon-ai-platform.com`
   * - `environment_1` corresponds to `https://engine-staging.solomon-ai-platform.com`
   * - `environment_2` corresponds to `http://localhost:8787`
   */
   ...
   *
   * Defaults to process.env['WORKSPACE_FINANCIAL_BACKEND_SDK_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['WORKSPACE_FINANCIAL_BACKEND_SDK_LOG'] or 'warn' if it isn't set.
   */
...
export class WorkspaceFinancialBackendSDK {
   ...
   * API Client for interfacing with the Workspace Financial Backend SDK API.
   *
   * @param {string | null | undefined} [opts.bearerToken=process.env['WORKSPACE_FINANCIAL_BACKEND_SDK_BEARER_TOKEN'] ?? null]
   * @param {Environment} [opts.environment=production] - Specifies the environment URL to use for the API.
   * @param {string} [opts.baseURL=process.env['WORKSPACE_FINANCIAL_BACKEND_SDK_BASE_URL'] ?? https://engine.solomon-ai-platform.com] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('WORKSPACE_FINANCIAL_BACKEND_SDK_BASE_URL'),
    ...
    this.timeout = options.timeout ?? WorkspaceFinancialBackendSDK.DEFAULT_TIMEOUT /* 1 minute */;
    ...
    const customHeadersEnv = readEnv('WORKSPACE_FINANCIAL_BACKEND_SDK_CUSTOM_HEADERS');
    if (customHeadersEnv) {
...
WorkspaceFinancialBackendSDK.APIGocardless = APIGocardless;
WorkspaceFinancialBackendSDK.APIPlaid = APIPlaid;
WorkspaceFinancialBackendSDK.APIInstitutions = APIInstitutions;
WorkspaceFinancialBackendSDK.APIRates = APIRates;
